...
(
	...
	[string]
	# Subscription id to log into Azure with
	$subscriptionId,
	...
	[string]
	# Username that has access to the specified subscription
...
if ($PSVersionTable.PSVersion.Major -eq 5) {
	...
	# Import the necessary modules for PowerShell 4
	Import-Module "C:\Program Files (x86)\Microsoft SDKs\Azure\PowerShell\ResourceManager\AzureResourceManager\AzureRM.Profile"
	Import-Module "C:\Program Files (x86)\Microsoft SDKs\Azure\PowerShell\ResourceManager\AzureResourceManager\AzureRM.Resources"
} else {

	Write-Verbose -Message "PowerShell versions less that 4 are not supported"
	break
}

# Disable Azure DataCollection otherwise the cmdlets prompt for information
Disable-AzureRmDataCollection

# Create a PSCredential to pass to the login command
$secpasswd = ConvertTo-SecureString $subscriptionPassword -AsPlainText -Force
...
$credentials_path = "{0}/.azure/credentials" -f $homepath

# Ensure the home path exists
if (!(Test-Path -Path (Split-Path -Path $credentials_path -Parent))) {
	New-Item -type directory -Path (Split-Path -Path $credentials_path -Parent)
}

if (Test-Path -Path $credentials_path) {

	# Move the existing file to a backup
	$backup_path = "{0}/.azure/credentials.bak-{1}" -f $homepath, ([int][double]::Parse((get-date -uformat %s)))
...
$CredentialsFile = New-Item $credentials_path -ItemType file -Force
